...
export default class Ball {
  ...
  render(svg, player1, player2) {
    ...
    if (this.rightPaddleCollided) {
      // ie. this is called until another paddle hits or a reset
  ...
  paddleCollision(player1, player2) {
    if (this.vx > 0) {
      let paddle = player2.coordinates(
        ...
        player2.width,
        player2.height
      ); // returns an array with coordinates for paddle in space
      ...
      if (
        ...
        if (this.vy > 0) {
          // on paddle collision ball speed increases
  ...
  backspin(player, svg) {
    // this function will only alter ball path if this.ballSpinConstant > 0
    ...
    if (this.ballSpinConstant !== 0) {
      ...
      if (this.bounceDecay < 1.2) {
        // only a max of 2 bounces is allowed
        this.ballSpinConstant = 0;
      }
    } else {
